...
const userUpdate = async () => {
        ...
        await Promise.all(user_ids.map(async (user_id) => {
            try {
                const [updated_user, user_leagues] = await Promise.all([
                    ...
                    axiosInstance.get(`https://api.sleeper.app/v1/user/${user_id}/leagues/nfl/${process.env.SEASON}`),
                ]);
                updated_users.push({
                ...
                leagues.push(...user_leagues.data.filter((l) => !leagues.some((league) => league.league_id === l.league_id)));
            }
            catch (error) {
    ...
    if (updated_users.length > 0) {
        await pool.query(`INSERT INTO adp__users (user_id, username, avatar) 
     ...
     DO UPDATE SET 
        ...
        updated_at = CURRENT_TIMESTAMP`, updated_users.flatMap((user) => [
            user.user_id,
            user.username,
            user.avatar,
        ]));
    }
    const league_ids_array = Array.from(new Set(leagues.map((league) => league.league_id)));
    // Check which league_ids already exist in database
    const existing_leagues = await pool.query(`SELECT league_id 
         FROM adp__leagues 
         WHERE league_id = ANY($1)`, [league_ids_array]);
    const new_leagues = leagues.filter((league) => !existing_leagues.rows.some((l) => l.league_id === league.league_id));
    if (new_leagues.length > 0) {
        console.log(`Inserting ${new_leagues.length} new leagues into database`);
        await pool.query(`INSERT INTO adp__leagues (league_id, name, avatar, season, settings, scoring_settings, roster_positions, status) 
     VALUES ${new_leagues
     ...
     DO UPDATE SET 
       ...
       status = EXCLUDED.status;`, new_leagues.flatMap((league) => [
            league.league_id,
            league.name,
            league.avatar,
            league.season,
