...
import { League, User, Draft } from "../lib/types.js";
...
const leagueUpdate = async () => {
      ...
      league_ids.map(async (league_id: string) => {
        try {
          ...
          league_drafts.data
            ...
            .forEach(async (draft: Draft) => {
              try {
                ...
                console.error(
                  `Error processing draft ${draft.draft_id}:`,
          ...
          users.push(
            ...league_users.data
              .filter(
                (user: User) => !users.some((u) => u.user_id === user.user_id)
              )
              .map((user: User) => ({
  ...
  if (updated_leagues.length > 0) {
    await pool.query(
      `INSERT INTO adp__leagues (league_id, name, avatar, season, settings, scoring_settings, roster_positions, status) VALUES ${updated_leagues
        .map(
          (_, i) =>
        ...
        .join(
          ", "
        )} ON CONFLICT (league_id) DO UPDATE SET name = EXCLUDED.name, avatar = EXCLUDED.avatar, season = EXCLUDED.season, settings = EXCLUDED.settings, scoring_settings = EXCLUDED.scoring_settings, roster_positions = EXCLUDED.roster_positions, status = EXCLUDED.status, updated_at = NOW()`,
      updated_leagues.flatMap((league) => [
        league.league_id,
        league.name,
        league.avatar,
        league.season,
  ...
  const existing_users = await pool.query(
    `SELECT user_id FROM adp__users WHERE user_id = ANY($1)`,
    [user_ids_array]
  );
  ...
  if (new_users.length > 0) {
    ...
    await pool.query(
      `INSERT INTO adp__users (user_id, username, avatar) VALUES ${new_users
        .map((_, i) => `($${i * 3 + 1}, $${i * 3 + 2}, $${i * 3 + 3})`)
        .join(", ")}`,
      new_users.flatMap((user) => [user.user_id, user.username, user.avatar])
    );
  }
